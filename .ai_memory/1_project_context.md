...
- 成本核算功能新增独立 `成本核算` 页；Go/Python 后端都提供 `/api/cost/summary`、`/api/cost/rules`，配置持久化在工具自建的 `api_tools_channel_costs` 表中，按 `logs.channel_id + logs.model_name` 聚合，支持映射到同一上游模型，并按“基础价格 × 渠道倍率”计算实际成本。
