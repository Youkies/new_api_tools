...
// GetDashboardSnapshot returns the dashboard first-screen data from one consistent endpoint.
func (s *DashboardService) GetDashboardSnapshot(period string, trendDays, topLimit int, noCache bool) (map[string]interface{}, error) {
	if trendDays <= 0 {
		trendDays = 7
	}
	if topLimit <= 0 {
	...
	cm := cache.Get()
	cacheKey := fmt.Sprintf("dashboard:snapshot:%s:%d:%d", period, trendDays, topLimit)
	if !noCache {
		var cached map[string]interface{}
		if found, _ := cm.GetJSON(cacheKey, &cached); found {
	...
	models, err := s.GetModelUsage(period, 8, noCache)
	if err != nil {
		models = []map[string]interface{}{}
	}
	topUsers, err := s.GetTopUsers(period, topLimit, noCache)
	if err != nil {
	...
	if period == "24h" {
		trendKind = "hourly"
		trends, err = s.GetHourlyTrends(24, noCache)
	} else {
		trends, err = s.GetDailyTrends(trendDays, noCache)
	}
	if err != nil {
		trends = []map[string]interface{}{}
	}

	freshness := s.GetDataFreshness(snapshotTime)
	result := map[string]interface{}{
		"period":          period,
		...
		"models":          models,
		"trends":          trends,
		"trends_kind":     trendKind,
		"top_users":       topUsers,
		"data_freshness":  freshness,
		"refresh_hint":    s.GetRefreshHint(period, freshness),
		"data_source":     "snapshot",
		"rollup_enabled":  false,
		"rollup_note":     "live logs queries with cache; rollup table is a later optimization",
		"generated_at":    snapshotTime,
		"top_users_limit": topLimit,
	}
...
// GetDataFreshness returns lightweight metadata describing how fresh logs-derived data is.
func (s *DashboardService) GetDataFreshness(snapshotTime int64) map[string]interface{} {
	row, err := s.db.QueryOneWithTimeout(5*time.Second, `
	...
	maxCreatedAt := toInt64(row["logs_max_created_at"])
	lagSeconds := int64(0)
	if maxCreatedAt > 0 && snapshotTime >= maxCreatedAt {
		lagSeconds = snapshotTime - maxCreatedAt
	}
	...
	return map[string]interface{}{
		"source":              "live_query",
		"logs_max_created_at": maxCreatedAt,
		"lag_seconds":         lagSeconds,
		"total_logs":          toInt64(row["total_logs"]),
		"status":              status,
	}
}

// GetRefreshHint provides a simple refresh recommendation based on data volume and freshness.
func (s *DashboardService) GetRefreshHint(period string, freshness map[string]interface{}) map[string]interface{} {
	totalLogs := toInt64(freshness["total_logs"])
	recommended := int64(60)
	reason := "normal"

	switch {
	case totalLogs >= 1000000:
		recommended = 300
		reason = "large_logs_table"
	case totalLogs >= 300000:
		recommended = 120
	...
	return map[string]interface{}{
		...
		"reason":                         reason,
		"force_refresh_requires_confirm": totalLogs >= 300000,
	}
}
